...
4. Each payline is read **right to left** from reel 5. Three or more matching symbols (WILD substitutes) pay from the paytable; only the best win per line is paid.
...
| --- | --- | --- |
| `PAY_DIRECTION` | `right` | The written brief says right-to-left. Reference screenshot 1 shows a left-to-right win (A-W-W on reels 1-3). Flip to `left` and re-run `engine/tune.py` if that was the intent. |
