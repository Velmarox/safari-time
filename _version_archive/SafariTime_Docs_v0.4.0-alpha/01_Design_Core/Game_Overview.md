...
3. **Expanding WILD:** WILD lands only on reels 2, 3 and 4 - never on the first or last reel (3, 4 and 3 stops per 100 respectively). If a WILD is visible anywhere on a reel, the whole reel becomes WILD before pays are evaluated.
...
- Played at the triggering bet on the **feature strips**, which carry fewer WILDs than the base game (1, 2 and 1 stops per 100 on reels 2-4, against 3-4-3). A feature WILD locks its reel for every remaining spin, so base-game density would make three locked reels routine; the leaner strips keep the feature worth about 13.5 points of RTP. The reference game likewise says its bonus reels differ from its main reels.
...
| `BASE_COUNTS` WILD | `0-3-4-3-0` | The RTP dial. Wild density compounds across adjacent middle reels: 3-3-3 gives ~64% base, 4-4-4 ~88%, 5-5-5 ~117% with this paytable. 3-4-3 with the outer reels' Lions/Elephants trimmed lands ~72%. |
...
| `FEATURE_COUNTS` WILD | `0-1-2-1-0` | Own strips since v0.4.0. 1-1-1 makes the feature worth ~9 points, 1-2-1 ~13.5, 2-2-2 ~25 - the last would push the total to ~104%, so the feature is deliberately the smaller share. |
