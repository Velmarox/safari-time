...
- Base game: P(WILD visible) = 0.09 / 0.12 / 0.09 on reels 2 / 3 / 4 (3, 4 and 3 Wild stops, each visible from 3 stops); reels 1 and 5 carry no WILD (`WILD_REELS`)
...
The feature strips are leaner in Wilds because a feature Wild locks its reel for the rest of the feature (section 4).
...
- **Expanding Wild density is the dominant lever and it compounds.** With everything else equal, 3 Wilds on each middle reel gives ~64% base line RTP, 4 gives ~88%, 5 gives ~117%. Two adjacent expanded reels turn every line into a 3+ chain, which is why the response is convex. The shipped layout is 3-4-3 (~72%).
- **Outer-reel high symbols are the fine adjustment.** Every chain is anchored on reel 1, so one Lion or Elephant stop more or less on reels 1 and 5 moves the base by roughly half a point; the same change on a middle reel moves it less.
- **The feature has its own dial.** Feature Wilds lock, so their density sets the feature's share: 1-1-1 is worth ~9 points, 1-2-1 ~13.5 (shipped), 2-2-2 ~25.
