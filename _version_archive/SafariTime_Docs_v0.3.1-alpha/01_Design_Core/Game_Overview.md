...
- On PLAY every reel rolls. Reel 1 lands 0.8 s later and each following reel keeps rolling until **exactly one second after the previous landing**, so a full spin settles over about 4.8 s. While the reels roll the PLAY button reads **STOP**; pressing it lands every remaining reel immediately, in order. Timings are constants at the top of the spin code (`FIRST_LAND_MS`, `LAND_INTERVAL_MS`, `LAND_MS`, `FAST_MS`).
