...
- Reels roll and land **one at a time**, left to right, with a pause between landings so anticipation can build. Once two AFRICA are showing, the remaining reels roll noticeably longer. While the reels roll the PLAY button reads **STOP**; pressing it lands every remaining reel immediately. Timings are constants at the top of the spin code (`ROLL_MS`, `PAUSE_MS`, `ANTICIPATION_MS`, `FAST_MS`).
